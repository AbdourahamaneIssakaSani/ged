    ...
    def __call__(self, field, **kwargs):
        ...
        for subfield in field:
            if self.prefix_label:
                html.append('<li>%s %s</li>' % (subfield.name, subfield()))
            else:
                html.append('<li>%s %s</li>' % (subfield(), subfield.name))
    ...
    def __call__(self, field, **kwargs):
        ...
        for subfield in field:
            if subfield.type in ('HiddenField', 'CSRFTokenField'):
                ...
                html.append('<tr><th>%s</th><td>%s%s</td></tr>' % (text_type(subfield.name), hidden, text_type(subfield)))
                hidden = ''
        if self.with_table_tag:
            html.append('</table>')
        if hidden:
...
class SubmitInput(Input):
    ...
    def __call__(self, field, **kwargs):
        kwargs.setdefault('value', field.name.text)
...
class Option(object):
    ...
    """
    def __call__(self, field, **kwargs):
        return Select.render_option(field._value(), field.name.text, field.checked, **kwargs)
